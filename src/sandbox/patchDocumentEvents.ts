...
// 所有子应用的document事件都绑定在documentEventMap上, 特别注意
export const documentEventMap = new Map<string, eventMap>()

// 主要是添加了对应的记录处理 -- documentEventMap
...
export function releaseAppDocumentEvents(appName: string) {
    ...
    for (const [type, arr] of curMap) {
        for (const {listener, options} of arr) {
            originalDocumentRemoveEventListener.call(originalDocument, type as string, listener, options)
        }
